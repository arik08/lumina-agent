...
export function AdminMcpPanel() {
  ...
  const approve = async (revisionId: string, revision: number) => {
    if (!selected) return;
    if (approveConfirmId !== revisionId) {
      setApproveConfirmId(revisionId);
      setDisableConfirmId(null);
      return;
    }
    setApproveConfirmId(null);
  ...
  const disable = async () => {
    if (!selected) return;
    if (disableConfirmId !== selected.id) {
      setDisableConfirmId(selected.id);
      setApproveConfirmId(null);
      return;
    }
    setDisableConfirmId(null);
