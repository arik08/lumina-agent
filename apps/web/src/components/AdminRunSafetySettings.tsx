    ...
    <section className="settings-card settings-admin-card settings-run-safety-card" aria-labelledby="admin-run-safety-title">
      ...
      {runSafety && (
        <form className="admin-run-safety-form" onSubmit={(event) => void saveRunSafety(event)}>
          ...
          <label><span>최대 실행 시간</span><div className="admin-run-safety-input"><input type="number" min="30" max="525600" step="1" value={runSafety.maxElapsedMinutes} onChange={(event) => setRunSafety({ ...runSafety, maxElapsedMinutes: Number(event.currentTarget.value) })} /><span>분</span></div><small>답변·승인·일시 정지 대기 시간 제외</small></label>
