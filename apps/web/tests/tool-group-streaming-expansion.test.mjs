...
test("every running tool spinner follows its label while duration and chevron stay rightmost", async () => {
  ...
  assert.match(app, /<span className="tool-call-label-with-status model-processing-label">\s*<span className="tool-call-label">\{running \? "Thinking\.\.\." : "Thinking"\}<\/span>\s*\{running \? <LoaderCircle/s);
