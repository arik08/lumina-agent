...
test("every running tool spinner follows its label while duration and chevron stay rightmost", async () => {
  ...
  assert.match(app, /<span className="tool-call-label-with-status model-processing-label">\s*<span className="tool-call-label">Thinking\.\.\.<\/span>\s*\{running\s*\? <LoaderCircle/s);
