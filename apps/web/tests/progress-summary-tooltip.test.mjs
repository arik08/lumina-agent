...
test("collapsed work plan becomes a thin expandable bar without a hover tooltip", async () => {
  ...
  assert.match(app, /<span className="current-step"[^>]*>[\s\S]*?latestProgressSummary \? userFacingSystemText\(latestProgressSummary\.text\) : runStatusLabel\(activeRun\.status\)/);
