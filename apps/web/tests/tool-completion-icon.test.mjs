...
test("completed icon is hidden for tool and model-processing rows", async () => {
  ...
  assert.match(app, /<span className="tool-call-label-with-status">\s*<span className="tool-call-label">\{execution\.label \|\| execution\.toolName\}<\/span>/);
  assert.match(app, /<span className="tool-call-label-with-status model-processing-label">/);
