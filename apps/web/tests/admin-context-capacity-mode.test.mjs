...
test("long-context models default to a named 85%-safe mode with an opt-in maximum mode", async () => {
  ...
  assert.match(app, /표준 모드는 272K 장문 요금 경계의 85%인 231,200 토큰에서 압축합니다/);
  assert.match(app, /비용 절약 모드는 272K 장문 요금 경계의 85%인 231,200 토큰에서 압축합니다/);
  assert.match(app, /standard_context_compaction_reserve_tokens/);
  assert.match(app, /"자동 압축 안전 여유"/);
  assert.match(app, /value=\{selectedAdminSettingsModel\.contextCapacityMode \?\? "standard"\}/);
  assert.match(app, /`최대 컨텍스트 · \$\{selectedAdminSettingsModel\.maximumContextWindow\.toLocaleString\(\)\} 토큰 활용`/);
  assert.match(app, /`최대 · \$\{selectedAdminSettingsModel\.maximumContextWindow\.toLocaleString\(\)\} 토큰 \(고비용\)`/);
