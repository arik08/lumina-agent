...
test("an uncached session renders a restoring state instead of the new-chat welcome", async () => {
  ...
  assert.match(app, /\{restoringActiveConversation && \([\s\S]*?className="conversation-loading-overlay" role="status" aria-live="polite"/);
  assert.match(app, /<LoaderCircle className="is-running" size=\{17\} \/> 대화를 불러오고 있습니다\./);
  assert.doesNotMatch(app, /restoringActiveConversation && <div className="conversation-loading"/);
