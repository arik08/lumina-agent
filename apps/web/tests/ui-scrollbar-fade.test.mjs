...
test("session scrollbar fades from its active strength instead of snapping", async () => {
  ...
  assert.match(styles, /@property --session-scrollbar-strength \{[^}]*syntax: "<percentage>";[^}]*initial-value: 17%;/s);
  ...
  assert.match(styles, /\.session-list \{[^}]*--session-scrollbar-strength: 17%;[^}]*transition: --session-scrollbar-strength var\(--scrollbar-fade-duration\) linear;/s);
