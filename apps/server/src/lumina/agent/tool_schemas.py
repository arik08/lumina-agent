...
_REQUEST_USER_INPUT_TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            "Pause the Run for a compact round of user questions. Call this tool by "
            "itself and only under the active clarification-mode contract. Group independent, "
            "currently answerable questions. Each question object must ask for exactly one fact or "
            ...
            "For an interview or intake, include the current frontier of independent high-value "
            "decisions in one bundle instead of intentionally splitting that frontier across "
            "repeated submit-and-wait cycles. Use a later call when earlier answers unlock or "
            "materially reshape dependent decisions. Never repeat a resolved question. A single "
            "bundle can contain up to twenty questions; there is no cumulative Run limit, so use "
            "another bundle when submitted answers leave material dependent decisions unresolved. "
            "Stop asking when the material decisions are actionable. The UI "
...
_REPORT_TOOL_SCHEMA: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "For numeric-dense HTML reports, apply a visualization coverage gate: comparable "
            "values, periods, categories, proportions, rankings, or score dimensions should "
            "produce at least one substantive chart instead of a document made only of prose, "
            "tables, KPI cards, or CSS progress bars. Prefer Apache ECharts for interactive "
            "data charts and inline SVG for small bespoke static visuals. "
            ...
            "properties": {
                ...
                "html_source": {
                    ...
                    "description": (
                        ...
                        "event handlers are supported for executable interactive HTML. When "
                        "the evidence contains at least two decision-relevant comparable numeric "
                        "values, periods, categories, proportions, ranks, or score dimensions, "
                        "include a substantive ECharts or inline-SVG chart unless the values are "
                        "incomplete, differently defined, or not comparable; state that concrete "
                        "exception in the report. Tables, KPI cards, badges, and CSS progress bars "
                        "do not satisfy this chart requirement. For ECharts, use a pinned 6.x "
                        "runtime, responsive containers with explicit height and resize handling, "
                        "a readable no-script/load-failure fallback, and exact values in labels "
                        "or a nearby table. Reserve title, subtitle, legend, and plot as separate "
                        "non-overlapping vertical bands: set explicit component positions and keep "
                        "the grid top below the measured title/subtitle and wrapped legend bounds "
                        "at desktop and narrow widths. Never place a legend over title or subtitle "
                        "text. Keep report table body text at least 14px on screen; preserve dense "
                        "tables with wrapping and horizontal overflow instead of font shrinking, "
                        "CSS transforms, or zoom. Use a "
                        ...
                        "expand/zoom controls. Put the diagram source directly in that `.mermaid` "
                        "element, never in a plain `pre` block. For categorical or hierarchical Mermaid "
...
_WEB_FETCH_TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "page locators and returns up to 50 pages per call. Only set page_start/page_end "
            "for a confirmed PDF when reading a specific or later range. When the user "
            "supplied the URL, fetch it directly without a preliminary web_search; otherwise "
            "fetch only the best sources shortlisted from search. Returned content is "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "page_start": {
                    ...
                    "minimum": 1,
                    "description": (
                        "First confirmed-PDF page to extract (1-based). "
                        "Omit for ordinary web pages."
                    ),
                },
                "page_end": {
                    "type": "integer",
                    "minimum": 1,
                    "description": (
                        "Last confirmed-PDF page to extract (inclusive, at most 50 pages). "
                        "Omit for ordinary web pages."
                    ),
                },
            },
