...
RICH_CHAT_RENDERING_CONTRACT = (
    ...
    "`classDef` and `class` assignments. Give equivalent roles or levels equivalent "
    "treatment, distinguish materially different groups, and avoid assigning colors by "
    "node order. Do not rely on the viewer to infer or reassign semantic colors. "
...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "changes, and mark finished steps `completed`. If you create a plan, treat "
    "composing and streaming the final user-visible answer as part of the work. "
    "Include a concrete answer-writing step tied to the requested outcome, keep that "
    "step `in_progress` while producing the answer, and do not mark the whole plan "
    "complete first; the runtime marks it `completed` when the Run completes. "
