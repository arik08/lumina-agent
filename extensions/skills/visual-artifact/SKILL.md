...
- For process maps and dense diagrams, use semantic color groups so the reader can scan the system at a glance: blue `#3288bd` for standards/requests/reports, teal `#66c2a5` for operations/market/planning, orange `#f46d43` or amber `#fdae61` for investment/CAPEX/strategic decisions, red `#d53e4f` for risk/issues, and purple `#5e4fa2` for governance/approval. Prefer palette-tinted pale fills with crisp colored borders and readable text; use stronger fills only for start/end, warnings, or key status nodes.
