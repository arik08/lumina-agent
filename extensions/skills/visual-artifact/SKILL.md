...
- For process maps and dense diagrams, infer a coherent color system from the actual subject instead of applying a fixed role-to-color lookup. Use the designated palette to group nodes by the relationships, roles, levels, phases, or boundaries that matter in that specific diagram; equivalent nodes must receive equivalent treatment, while materially different groups should remain easy to distinguish. Encode those choices directly in Mermaid source with `classDef` and `class` assignments so the saved diagram remains visually stable. Never assign colors merely by node order, and do not rely on Lumina's renderer to infer or replace the intended semantics. Prefer palette-tinted pale fills with crisp colored borders and readable text; use stronger fills only when the diagram's own context warrants them.
