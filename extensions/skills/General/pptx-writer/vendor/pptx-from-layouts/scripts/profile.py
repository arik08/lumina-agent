...
def run_command(cmd: list, description: str) -> tuple[bool, str]:
    ...
    try:
        ...
        pathsep = os.pathsep
        skill_paths = pathsep.join(
            str(path) for path in [_SKILL_DIR, _SKILL_DIR / 'lib', _SKILL_DIR / 'schemas', _SCRIPT_DIR]
        )
        env['PYTHONPATH'] = f"{skill_paths}{pathsep}{pythonpath}" if pythonpath else skill_paths
