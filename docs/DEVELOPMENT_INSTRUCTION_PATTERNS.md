...
7. 요청 범위를 바꾸는 선택이 필요하면 추측으로 확대하지 않고 사용자에게 확인합니다. 충돌 통지만으로 작업을 멈추지는 않으며, 자율 진행과 승인 판단은 루트 `AGENTS.md`의 해당 절을 따릅니다.
...
작업 유형별 완료 조건을 기준으로 아래 질문 중 적용되는 항목을 점검합니다. 적용되지 않는 항목과 환경 때문에 수행하지 못한 필수 검증을 구분합니다. 필수 검증이 막히면 허용된 대안을 시도하고, 수행한 작업과 검증 대기 범위를 분리해 보고합니다. 실제 UI 변경의 격리 브라우저 검증 요건은 유지하며, 미검증 항목을 완료로 표시하지 않습니다.
