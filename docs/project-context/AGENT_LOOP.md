...
8. 결과를 크게 바꿀 모호함이 있으면 `request_user_input`을 단독 호출하여 질문 묶음을 저장하고 `awaiting_input`에서 계정 소유자의 답변을 기다립니다. 한 묶음은 최대 20문항이며, 답변에 종속된 중요한 후속 결정이 있으면 같은 Run에서 새 묶음으로 이어갈 수 있습니다. Run 전체의 누적 문항 제한은 없습니다.
