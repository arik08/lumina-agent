...
일반 `.py`와 `insane-search` 같은 보조 script는 기본 `standard` profile을 사용하며 10분 상한과 5MB Skill package 상한을 유지합니다. 장시간 또는 계산량이 큰 Python program을 포함한 Skill은 관리자 opt-in인 `heavy` profile을 사용합니다. `heavy`는 임의 Artifact에는 허용하지 않고 고정된 활성 Skill에만 허용하며, 관리자가 `LUMINA_PYTHON_EXECUTION_EXECUTABLE`로 전용 Python 또는 virtual environment를 지정하고 `LUMINA_PYTHON_HEAVY_EXECUTION_ENABLED=true`를 설정한 경우에만 실행합니다. 시간 hard cap은 24시간이고 환경 설정으로 더 낮출 수 있습니다. Run 전체 deadline이 더 짧으면 Run deadline이 우선하므로 장시간 배포는 `LUMINA_RUN_TIMEOUT_SECONDS`도 요청 시간 이상으로 설정해야 합니다. Skill source package는 `heavy`도 기존 5MB 상한을 유지합니다. 200MB급 program과 native·Python dependency는 Run 중 설치하거나 package에 복사하지 않고 전용 runtime image 또는 virtual environment에 미리 고정합니다. Skill에는 사용자에게 제안할 입력 schema·질문 순서·검증 규칙·얇은 Python wrapper·결과 해석 지침을 둡니다. 모델이 필수 값을 모두 수집하면 하나의 JSON object를 `input_json`으로 전달하고, wrapper는 stdin에서 읽어 관리형 program을 호출한 뒤 stdout에 결과를 출력합니다. Lumina는 stdout을 제한·정리해 Tool 결과로 반환하고 모델은 이를 data로 분석합니다. 큰 model/data가 필요한 Skill을 실제 도입할 때는 package 제한을 늘리지 않고 권한 검증된 Artifact·object storage 입력 계약을 함께 구현합니다.
