...
5. 보고서가 아닌 Node도 사용자가 직접 읽을 수 있는 Markdown 중간보고서를 저장합니다. 확인한 사실·근거·계산·불확실성과 다음 Node가 알아야 할 내용을 담되, 서론·Executive Summary·맺음말 같은 최종 보고서용 문장 광택은 만들지 않습니다. Run이 `create_report` 또는 `write_file`로 상세 문서를 만들고 채팅에는 짧은 완료 안내만 남긴 경우에도 상세 문서 원문을 Node의 대표 출력으로 보존합니다.
