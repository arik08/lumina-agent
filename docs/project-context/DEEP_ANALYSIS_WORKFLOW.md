...
5. 보고서가 아닌 Node는 확인한 사실·근거·계산·불확실성과 다음 Node가 알아야 할 내용을 압축한 Markdown 인계물을 저장합니다. 서론·Executive Summary·맺음말 같은 최종 보고서용 문장 광택은 만들지 않습니다.
...
중간 Node 출력은 Agent가 작성한 Markdown을 그대로 보존하고 렌더링합니다. 최종 보고서 Node는 Mission에서 선택한 형식의 원문과 파일 경로를 보존하며, 별도의 중복 원문 영역은 만들지 않습니다.
