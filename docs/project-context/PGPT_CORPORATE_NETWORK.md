...
- GPT-5.6 Sol·Terra·Luna의 공식 전체 Context window와 검증된 입력 Token 상한을 별도 capability와 관리자 설정으로 관리합니다. 기본 모드는 272K, 최대 모드는 1.05M이며, 관리자 화면에서 입력 상한을 바꿔도 공식 전체 Context 값은 유지합니다. Lumina 자체 System prompt와 Tool schema도 입력을 소비하므로 실제 대화 Context 예산에서는 두 값 중 작은 한도에서 Tool schema와 안전 여유를 추가로 제외합니다.
