...
새 Mission의 초기 설정은 일반 Composer와 같은 `분석 범위`, `답변 분량`, `출력 방식`, `출력 토큰` 계약을 사용합니다. MISSION 상세 패널의 최종 산출물 형태는 Markdown을 기본값으로 사용하고 HTML을 추천값으로 제공하며 사용자가 같은 입력란에 원하는 형태를 직접 적을 수 있습니다. 이 값은 마지막 Node가 아니라 Mission 실행 설정에 저장됩니다. HTML을 포함한 입력은 독립 실행 가능한 `.html` 원문으로 저장하고, 그 밖의 직접 입력값은 최종 보고서 작성 지시에 반영하면서 원문을 `.md`로 보존합니다. `분석 범위`는 모든 Node에 적용하지만 사용자가 선택한 `답변 분량`, `출력 토큰`과 최종 산출물 형태는 보고서 Node의 최종 산출물에 적용합니다. 보고서가 아닌 Node는 유형별 내부 상한 안에서 사실·근거·계산·불확실성과 다음 Node가 알아야 할 내용만 담은 Markdown 압축 인계물을 작성하고, 최종 보고서용 서론·요약·맺음말을 반복 생성하지 않습니다. Run이 `write_file`로 상세 문서를 만든 뒤 짧은 완료 안내를 반환해도 상세 원문을 사용자가 보는 Node 출력과 다음 Node 인계에 공통으로 사용하는 canonical 출력으로 보존합니다. 사용자는 실행 전 Workflow 편집 상태에서 각 Node의 작업 프롬프트를 확인하고 수정할 수 있습니다. 사용자는 새 문서를 Project 파일로 첨부하거나 `@`로 기존 파일·폴더·Artifact를, `$`로 Project에서 허용된 Skill·MCP를 명시할 수 있습니다. Backend는 선택한 참조의 version·revision·digest와 실행 설정을 Mission에 고정하고 각 Node Run snapshot에 전달하며, 명시적으로 선택한 Project 파일 revision만 Mission source manifest로 사용합니다. 선택한 자료가 없으면 source manifest는 비워 두고 Project의 다른 파일을 자동 포함하지 않습니다.
