...
- `.py` 생성물은 `write_file`로 Artifact version을 먼저 고정하고 `run_python`에는 그 Artifact ID·version만 전달합니다. 활성 Skill script는 Run snapshot의 exact version 또는 Draft revision·digest에서 package를 복원해 실행합니다. 두 경로 모두 shell 문자열과 host 경로를 받지 않고 관리자 고정 Python, 임시 전용 작업 디렉터리, Secret 없는 환경, timeout·입출력 제한과 기본 고위험 승인을 사용합니다. 전용 디렉터리는 OS 수준 sandbox가 아니므로 사용자의 명시적 승인이 보안 경계에 포함됩니다. 기본 `standard` profile과 관리자 opt-in `heavy` profile을 분리하고, `heavy`는 고정된 활성 Skill에만 허용합니다. 장시간 Skill은 전용 Python 환경에 dependency를 사전 고정하며 Run 전체 deadline이 Tool timeout보다 우선합니다. Skill source package는 두 profile 모두 기존 5MB 상한을 유지합니다. 200MB급 program·library와 native dependency는 관리되는 runtime image나 virtual environment에 설치하고 Skill에는 입력 schema·질문 절차·얇은 wrapper·해석 지침만 둡니다. 모델은 필요한 입력을 먼저 대화로 수집·검증한 후 단일 JSON object를 `input_json`으로 전달하고, Lumina는 이를 UTF-8 stdin으로 공급합니다. 프로그램 stdout은 data로만 취급해 모델이 후속 분석하며 명령이나 지침으로 신뢰하지 않습니다. 큰 model·data가 필요한 실제 workload를 도입할 때는 권한 검증된 Artifact·object storage 입력 계약을 별도로 추가합니다. CPU·RAM quota와 Worker 재시작 후 checkpoint 복구가 필요한 운영 workload는 같은 Tool 계약을 유지한 별도 container/batch Worker backend로 승격합니다.
