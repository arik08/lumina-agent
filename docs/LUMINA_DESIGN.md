...
| 심층분석 Mission Workflow | 구현 | 독립 메뉴와 Workspace Frontend, Project 귀속 Mission·Workflow revision·Node·Edge, 즉시 생성되는 빈 수동 Workflow와 선택적 AI 재생성, Node별 채팅 세션과 Run, Canvas·Inspector·실제 프롬프트·Markdown 출력·누적 비용·event replay·export를 제공합니다. Pattern·실행 중 재계획·Claim·Evidence·Quality Gate는 계약에서 제외합니다. |
...
새 Mission은 LLM 호출 없이 빈 수동 Workflow로 즉시 생성됩니다. 사용자는 Node 유형·제목·프롬프트와 Edge를 직접 구성하며, AI 초안이 필요할 때만 Workflow 재생성을 명시적으로 요청합니다. 실행기는 DAG 순서와 결과 전달만 담당합니다. 의존성이 모두 충족된 Node는 사용자·서버 동시 실행 한도 안에서 함께 시작하고, 합류 Node는 모든 선행 Node가 완료된 뒤 실행합니다. 활성 Mission 화면은 sequence replay가 가능한 SSE를 사용하고, 연속 event의 상세 snapshot 갱신을 짧게 합쳐 처리하며 최근 event 1,000개만 메모리에 유지합니다. 실행 중 재계획과 검증 장부 생성을 위해 별도 모델을 호출하지 않습니다.
