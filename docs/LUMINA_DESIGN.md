...
> 문서 상태: 통합 기준안
> 작성일: 2026-07-11
> 최종 동기화: 2026-07-20 (현재 source 작업 트리와 migration `0058` 기준)
...
| [`DEEP_ANALYSIS_WORKFLOW.md`](project-context/DEEP_ANALYSIS_WORKFLOW.md) | 독립 심층분석 Mission의 계약: Node별 채팅 세션, drag·connect Workflow, Edge 기반 결과 전달, Markdown 출력, 비용·Context·복구·API·event |
...
| [`KNOWLEDGE_SYSTEM_DESIGN.md`](project-context/KNOWLEDGE_SYSTEM_DESIGN.md) | 문서 단위 지식 저장, 선택 검색 도구, 사용 모드, passage·인용·선택 점수 계약 |
| [`KNOWLEDGE_TAG_TAXONOMY.md`](project-context/KNOWLEDGE_TAG_TAXONOMY.md) | 지식 문서, canonical 태그 사전·계층·별칭과 제안·승인 운영의 단계별 계약 |
| [`lumina_skill_management_design.md`](project-context/lumina_skill_management_design.md) | 개인 작업본, Creator·Owner 분리, Merge·Change Request·Publish·Rollback과 Skill Evolution 방향 |
...
2026-07-20 동기화 시점의 주요 상태는 다음과 같습니다.
...
아래 표는 2026-07-20의 migration `0001`~`0058`, 현재 route·service·Frontend source와 회귀 test로 확인한 실제 구현 범위입니다. `Implemented`는 source가 존재하고 해당 계약 test가 있다는 뜻이며 운영 규모·보안 심사·실사용 품질까지 완료되었다는 뜻은 아닙니다.
...
| 관리자 | 사용자 생성·조회·수정·비밀번호 reset·role/status 변경, 마지막 active admin 보호, 조직 범위 사용량 통계, 첫 호출·후속 호출·고정 prompt digest별 cached/uncached input과 cache hit ratio 집계, 다른 사용자 대화·Turn Set 감사 조회, audit event 검색, 공유 링크 목록과 token 비노출 강제 revoke, 조직 Run 안전 한도 설정과 비상 전체 중단 | `routes/admin.py`, `test_admin_api.py`, `admin-cache-monitoring.test.mjs`, `test_admin_share_revoke.py`, `test_run_safety.py` |
...
| Project File·Workspace | Project 파일 upload·목록·상세·rename·새 version·download·삭제, 안전한 상대 경로, content hash와 immutable version, DB commit 실패 시 storage cleanup, Composer와 Run의 exact file version 고정, 대용량 `@file`·`@folder` source-document index와 필요 구간 지연 조회, 문서 구조·질문 의도 기반 `explore_source_document` section 탐색과 정확한 원문 줄 검증, `read_file`·`write_file`·`glob` Project scope 강제 | `project_files/`, `tools/source_documents.py`, `tools/workspace.py`, `agent/executor.py`, `test_source_documents.py`, `test_project_workspace.py`, `test_workspace_tools.py`, `test_composer_run_references.py` |
| Conversation | favorite 우선 cursor 목록과 Sidebar 점진 로딩, 좋아요 filter·보존, whitespace-tolerant 제목 검색, SQLite FTS·portable fallback 기반 Message 본문 검색과 snippet, Turn Set 역방향 pagination·이전 묶음 선행 로딩, revision CAS와 no-op PATCH 차단, first Run 임시·모델 제목, Project 이동과 Session-owned asset 이동, 특정 Message 기준 분기, JSON·Markdown 내보내기, builtin Agent Frontend contract 저장·fallback | `routes/conversations.py`, `conversations/service.py`, `agent_frontends/`, migration `0056`, `test_conversation_listing.py`, `conversation-list-pagination.test.mjs`, `test_conversation_branch_export.py`, `test_agent_frontend_contract.py` |
...
| Context·Memory | recoverable compaction, source Message ID 기반 현재 Conversation 원문 검색·부분 읽기, Tool Call/Result pair·side effect 보존, 실패 시 원 Context 유지, 모델별 Context budget, 사용자 Memory 후보·자동/확인/끔 mode, 민감정보 차단, relevant subset recall, LLM 최적화와 provenance 병합, Project learning proposal approve·reject·apply·rollback | `context/`, `tools/conversation_context.py`, `memories/`, `project_memories/`, `test_context_compaction_memory_learning.py`, `test_project_memory_learning.py` |
| Provider | Mock·P-GPT·OpenAI Responses·Anthropic·Gemini·OpenAI Compatible adapter, multimodal image 입력, Tool roundtrip, typed·redacted 오류, 공통 base URL·Retry-After 검증, 관리자 Model discovery·명시 활성화·공식 Context와 실측 입력 상한 분리, 장문 가격 경계가 있는 모델의 기본 272K `표준`·명시 선택 1.05M `최대` 용량 mode, revision 기반 사용자·Project 실행 선택 저장, Codex ChatGPT OAuth App Server·warm client·transport retry·API key 제거, 사용자 hash prompt-cache routing | `providers/`, `routes/providers.py`, `routes/admin_providers.py`, `test_provider_http.py`, `test_model_output_limits.py`, `test_settings_revision_cas.py` |
...
| Frontend 내구성·상세 UI | `crypto.randomUUID` fallback, Backend readiness 연속 확인 뒤 reload, top-level React Error Boundary, Session 복원 loading, 명시한 새 채팅을 늦은 hydration이 덮어쓰지 않는 pending guard, 부가 화면 lazy loading, Composer 제어의 독립 component 경계와 stop, 대화 중 Model 변경의 prompt-cache 경고, Turn renderer 분리, 확인 질문 card, Plan 자동 open/terminal close, Tool group·duration·model exchange·write progress, source evidence, 공용 body Portal tooltip, Artifact panel·공유 viewer·clipboard fallback, 사용자별 채팅 폭·글꼴 설정 | `AppErrorBoundary.tsx`, `BackendConnectionGuard.tsx`, `ComposerControls.tsx`, `ConversationTurn.tsx`, `GlobalTooltip.tsx`, `use-lumina-workspace.ts`, `client-id.ts`와 `apps/web/tests/` |
...
### 1.5 2026-07-16~20 source 정합화 결과
...
| MCP wrapper | MCP definition payload와 상세 화면은 Agent Skills 표준 `metadata.lumina-source: skill-mcp:<slug>` wrapper 적용 여부를 함께 표시합니다. wrapper가 없으면 사용 지침이 Context에 들어가지 않음을 경고하며, 실제 연결이 없는 POSCO placeholder manifest·stub·wrapper는 builtin catalog처럼 남겨 두지 않습니다. |
...
| 심층분석 계약 | `deep-analysis`는 별도 Workspace Frontend와 Backend domain으로 실제 구현되었습니다. Mission 생성 시 일반 Composer와 같은 분석·답변·출력 설정, 선택 자료의 exact revision·digest와 빈 source manifest 의미를 고정합니다. 비보고서 Node는 사용자가 읽을 수 있는 중간보고서 겸 압축 인계물을, 보고서 Node는 사용자 출력 설정을 사용하며 start·pause·resume·cancel·retry·event replay·비용·Project 파일 export를 지원합니다. |
...
- 장기·대형 업무를 위한 독립 심층분석 Mission, Node별 채팅 세션과 Edge로 구성하는 시각 Workflow, 단위별 LLM 출력·실제 비용 보존
...
- 서로 다른 채팅 Session도 같은 인증 사용자와 동일한 고정 prompt prefix라면 캐시 라우팅을 재사용합니다. 정규화한 로그인 ID(email)의 비가역 hash를 사용자 scope로 만들고, Provider·Model·고정 system prompt·정렬된 Tool schema의 내용 hash를 결합한 `prompt_cache_key`를 모델 요청 시 생성합니다. 이메일 원문은 Provider 요청이나 로그에 넣지 않으며, 요청별 output mode·현재 Message·recall 같은 변동 정보는 안정된 system·Tool prefix 뒤에 둡니다. 한 Run이 시작된 뒤에는 Tool schema를 제거하거나 재정렬하지 않고 호출 상한은 실행 단계에서 거부하여 prefix를 유지합니다. Codex는 Direct Responses를 기본 전송으로 사용하고 동일한 `prompt_cache_key`에서 파생한 안정된 `session-id` 헤더로 새 Session도 같은 cache route에 보냅니다. Direct Responses 호환성 오류가 출력 전에 발생할 때만 App Server를 fallback으로 사용합니다.
- 첫 실제 Codex 요청에서 검증된 사용자·Model·고정 system prompt·Tool schema를 `prompt_cache_seed`로 저장합니다. Backend가 다시 시작되면 사용자·Model별 가장 최근 seed를 고르고 프로세스당 최대 4개를 백그라운드 Direct Responses 요청으로 prewarm하며, 시작 완료나 실제 Run dispatch를 기다리게 하지 않습니다. prewarm 출력은 사용자 대화에 저장하거나 노출하지 않고 cache usage만 기록합니다. 지침·Tool schema·Model이 바뀌면 static digest와 cache key가 달라져 새 seed가 생성되며 사용자·Model별 최근 seed는 최대 4개만 보존합니다.
...
- `.py` 생성물은 `write_file`로 Artifact version을 먼저 고정하고 `run_python`에는 그 Artifact ID·version만 전달합니다. 활성 Skill script는 Run snapshot의 exact version 또는 Draft revision·digest에서 package를 복원해 실행합니다. 두 경로 모두 shell 문자열과 host 경로를 받지 않고 관리자 고정 Python, 임시 전용 작업 디렉터리, Secret 없는 환경, timeout·입출력 제한과 기본 고위험 승인을 사용합니다. 전용 디렉터리는 OS 수준 sandbox가 아니므로 사용자의 명시적 승인이 보안 경계에 포함됩니다. 기본 `standard` profile과 관리자 opt-in `heavy` profile을 분리하고, `heavy`는 고정된 활성 Skill의 장시간 결정적 script에만 허용합니다. Run 전체 deadline이 Tool timeout보다 우선합니다. Agent Skills 표준에 없는 5MB package 상한을 Skill 규칙처럼 두지 않으며, catalog의 `name`·`description`, 활성화된 `SKILL.md`, 필요한 `scripts/`·`references/`·`assets/` 순으로 점진적으로 공개합니다. Python script는 내용을 모델 Context에 넣지 않고 직접 실행할 수 있습니다. 200MB급 회사 program·native dependency·별도 계산 service는 Skill package나 Backend 환경에 복사하지 않고 MCP server의 관리형 runtime에 version·image digest로 고정합니다. 표준 Skill wrapper는 사용자에게 제안할 입력 schema·질문 절차·검증 규칙·MCP Tool 호출법·결과 해석 지침을 제공하고, MCP Tool의 JSON schema가 실제 입력·출력 계약을 강제합니다. 모델은 필요한 입력을 대화로 수집·검증한 뒤 MCP Tool을 호출하고 반환값을 data로 분석합니다. bundled script를 `run_python`으로 실행하는 경우에도 stdout은 data로만 취급합니다. 큰 model·data에는 권한 검증된 Artifact·object storage 입력 계약을 추가하고, CPU·RAM quota와 Worker 재시작 복구가 필요한 운영 workload는 MCP 뒤의 container·batch Worker가 담당합니다.
...
Composer 하단의 `$` 호출 버튼 오른쪽에는 `자동 | 채팅 | 파일` 출력 방식 선택기를 둡니다. `자동`이 기본값이며 기존 Agent 판단과 명시된 파일 형식 요청을 따릅니다. `채팅`은 일반 요청의 최종 결과를 채팅 답변으로 반환하지만, 사용자가 보고서 작성을 명시하면 보고서 기본 형식인 HTML Artifact를 우선 생성합니다. `파일`은 최종 산출물을 Artifact로 생성하며 채팅에는 짧은 요약과 Artifact 연결만 남깁니다.

`채팅` 모드에서 보고서 작성 의도가 감지되면 Composer는 출력 방식 선택기 주변에 HTML Artifact로 생성된다는 안내를 표시합니다. 이 안내는 사용자가 채팅 답변만 원했는지 전송 전에 확인할 수 있도록 하는 보조 신호이며, 최종 의도 판정은 Backend가 담당합니다.
...
Mermaid는 완결된 fence만 parse하고 streaming 중에는 placeholder를 표시합니다. flowchart의 괄호가 있는 unquoted label처럼 안전하게 판별 가능한 문법과 authored `classDef root`처럼 Mermaid 내부 SVG class와 충돌하는 이름만 renderer 직전 보정하고, 작성자가 이미 인용했거나 의미가 불명확한 source는 임의 수정하지 않습니다. 큰 diagram은 viewport 기준 자동 fit, 단계식 zoom, pointer pan·drag scroll, scroll boundary 연결, reset과 keyboard close를 제공하며 header 전체를 명확한 확대 affordance로 사용합니다. 오류는 다른 본문을 깨뜨리지 않습니다.

Mermaid와 HTML Artifact의 chart·diagram·data accent는 사용자가 MyHarness에서 지정한 `#3288bd`, `#66c2a5`, `#e6f598`, `#d53e4f`, `#9e0142`, `#f46d43`, `#fdae61`, `#fee08b`, `#abdda4`, `#5e4fa2` palette를 기본으로 사용합니다. 별도 brand palette가 명시된 경우에만 이를 대체하며, 제품 UI의 단일 cobalt accent 규칙을 Artifact 시각화에 강제로 덮어쓰지 않습니다. 범주형·계층형 Mermaid flowchart는 최상위 의미 가지마다 하나의 hue family를 정하고 모든 하위 node에 같은 family를 적용하며, 계층 차이는 필요할 때만 같은 family 안의 밝기·채도로 표현합니다. Root·공유 node는 별도 anchor로 구분하고 authored class를 사용하면 모든 node를 빠짐없이 지정합니다. Red·coral·amber는 실제 risk·warning·status에만 사용하며 설명 없는 예외 강조색으로 두 번째 분류축을 섞지 않습니다. 의미는 label·group·structure만으로도 이해할 수 있어야 하고 node text와 fill은 최소 4.5:1 contrast를 유지합니다. Mermaid의 authored `classDef`는 보존하고, class가 없는 node와 pie·C4·gitGraph·XY chart 계열에는 이 기본 palette를 renderer가 적용합니다.
...
Agent Frontend code registry에는 범용 Agent와 `GeneralChatFrontend`를 builtin 상수로 등록합니다. `agent_id`, `agent_version`과 frontend contract version은 Conversation·Run snapshot에 저장하되, Agent catalog용 DB·installer·remote loader는 만들지 않습니다. 공통 Shell에는 별도의 builtin Workspace slot이 있으며 `deep-analysis`와 `knowledge`가 이 경계로 로드됩니다. 이 구현에서 실제로 확인된 최소 Shell slot과 typed contract만 유지하고 registry persistence, runtime loader와 범용 Plugin Framework를 먼저 만들지 않습니다.
...
`deep-analysis`는 일반 채팅과 다른 Mission 목록, Workflow Canvas, Node Inspector, 실행 기록과 비용 화면을 가지므로 이 경계를 사용하는 첫 특수목적 builtin Workspace Frontend입니다. Workflow Revision·Node·Edge와 Node별 채팅 세션 연결은 같은 선택 module의 Backend domain이 소유하고, 인증·Project 권한·Run·Queue·Provider·Tool 승인·Storage·usage 원시 계측·감사·replay는 Core를 재사용합니다. 심층분석을 위해 일반 채팅, `App.tsx`, Project 파일과 Run 내부에 module ID 조건문을 흩뿌리지 않습니다.

새 Mission은 LLM 호출 없이 빈 수동 Workflow로 즉시 생성됩니다. 사용자는 Node 유형·제목·프롬프트와 Edge를 직접 구성하며, AI 초안이 필요할 때만 Workflow 재생성을 명시적으로 요청합니다. 일반 Edge는 DAG 순서와 결과 전달을 담당합니다. 예외적인 `loop_back`은 정상 상하 경로가 존재하는 `validation`·`data_check` Node에서 선행 Node로만 연결하고, 좌우 포트와 바깥쪽 복귀선으로 표시하며 명시 조건과 최대 2~3회 반복으로 제한합니다. 사람과 AI가 만든 Loop는 동일한 저장 검증을 통과해야 하고, 실행 시 검증 Node 자체의 구조화 판단으로 해당 정상 경로만 다시 실행합니다. 의존성이 모두 충족된 Node는 사용자·서버 동시 실행 한도 안에서 함께 시작하고, 합류 Node는 모든 선행 Node가 완료된 뒤 실행합니다. 활성 Mission 화면은 sequence replay가 가능한 SSE를 사용하고, 연속 event의 상세 snapshot 갱신을 짧게 합쳐 처리하며 최근 event 1,000개만 메모리에 유지합니다. 실행 중 임의 재계획과 검증 장부 생성을 위해 별도 모델을 호출하지 않습니다.

새 Mission의 초기 설정은 일반 Composer와 같은 `분석 범위`, `답변 분량`, `출력 방식`, `출력 토큰` 계약을 사용합니다. MISSION 상세 패널의 최종 산출물 형태는 Markdown을 기본값으로 사용하고 HTML을 추천값으로 제공하며 사용자가 같은 입력란에 원하는 형태를 직접 적을 수 있습니다. 이 값은 마지막 Node가 아니라 Mission 실행 설정에 저장됩니다. HTML을 포함한 입력은 독립 실행 가능한 `.html` 원문으로 저장하고, 그 밖의 직접 입력값은 최종 보고서 작성 지시에 반영하면서 원문을 `.md`로 보존합니다. `분석 범위`는 모든 Node에 적용하지만 사용자가 선택한 `답변 분량`, `출력 토큰`과 최종 산출물 형태는 보고서 Node의 최종 산출물에 적용합니다. 보고서가 아닌 Node도 사용자가 직접 읽을 수 있는 Markdown 중간보고서를 작성합니다. 유형별 내부 상한 안에서 사실·근거·계산·불확실성과 다음 Node가 알아야 할 내용을 담되 최종 보고서용 서론·요약·맺음말을 반복 생성하지 않습니다. Run이 `create_report` 또는 `write_file`로 상세 문서를 만든 뒤 짧은 완료 안내를 반환해도 상세 원문을 사용자가 보는 Node 출력과 다음 Node 인계에 공통으로 사용하는 canonical 출력으로 보존합니다. 사용자는 실행 전 Workflow 편집 상태에서 각 Node의 작업 프롬프트를 확인하고 수정할 수 있습니다. 사용자는 새 문서를 Project 파일로 첨부하거나 `@`로 기존 파일·폴더·Artifact를, `$`로 Project에서 허용된 Skill·MCP를 명시할 수 있습니다. Backend는 선택한 참조의 version·revision·digest와 실행 설정을 Mission에 고정하고 각 Node Run snapshot에 전달하며, 명시적으로 선택한 Project 파일 revision만 Mission source manifest로 사용합니다. 선택한 자료가 없으면 source manifest는 비워 두고 Project의 다른 파일을 자동 포함하지 않습니다.

심층분석 model 요청은 공통 prompt-cache 계약을 사용하되 `Core → Mission 설명과 Project 자료 → 공통 선행 Node 출력 → 현재 Node 프롬프트` 순서로 조립합니다. 같은 fan-out 묶음은 Node별 값이 등장하기 전까지 동일한 byte-stable prefix를 사용하고 Context manifest의 `prefixHash`는 이 앞부분과 정확한 파일 version·content hash·dependency lineage를 함께 고정합니다. 합류 Node 입력은 직접 선행 Node의 대표 출력·보조 산출물을 먼저 안정 정렬하고 이전 공통 조상 산출물을 별도 구분하며, content hash·출처 기준 중복을 제거하되 충돌하는 결론의 provenance는 보존합니다. 비용은 실제 Node Run 사용량을 기준으로 집계합니다.

추가 최적화는 실제 Node Run의 Context 크기와 cache 효과를 측정한 뒤 도입합니다. 실행 중 Workflow 탐색이나 별도 LLM 판정은 기본 경로에 추가하지 않습니다.

Frontend Host는 Conversation 중심 slot과 builtin `Workspace Frontend Slot`을 분리합니다. `deep-analysis`와 `knowledge`는 독립 폴더에서 lazy load하고, Project 선택·primary navigation·sidebar 목록·본문 workspace의 공통 Shell 상태만 공유합니다. Conversation Frontend와 Workspace Frontend의 실제 공통점만 contract로 추출하며 `moduleKind`, package loader와 추상 base class를 일반화하지 않습니다.
...
### 21.2 현재 구현된 확장 table과 schema (`0002`~`0058`)
...
extensions.tags_json / installations.project_ids_json Skill 분류와 Project 설치 범위
announcement_receipts                              사용자별 공지 확인 상태
deep_analysis_missions / events / commands          Mission 상태, replay event와 idempotent 명령
deep_analysis_context_manifests / mission_file_links 선택 자료 snapshot과 파일 lineage
deep_analysis_workflow_revisions / nodes / edges    불변 Workflow revision과 DAG 실행 결과
deep_analysis_decisions / quality_gate_results      개발 중 생성된 결정·검증 record
deep_analysis_claims / evidence_references          개발 중 생성된 Claim·Evidence record
...
# 심층분석 Mission
GET    /api/projects/{project_id}/deep-analysis/missions
POST   /api/projects/{project_id}/deep-analysis/missions
GET    /api/deep-analysis/missions/{id}
PATCH  /api/deep-analysis/missions/{id}
DELETE /api/deep-analysis/missions/{id}?expected_revision={revision}
POST   /api/deep-analysis/missions/{id}/workflow/regenerate
POST   /api/deep-analysis/missions/{id}/revisions
PATCH  /api/deep-analysis/missions/{id}/draft
POST   /api/deep-analysis/missions/{id}/draft/activate
POST   /api/deep-analysis/missions/{id}/start
POST   /api/deep-analysis/missions/{id}/pause
POST   /api/deep-analysis/missions/{id}/resume
POST   /api/deep-analysis/missions/{id}/cancel
POST   /api/deep-analysis/missions/{id}/retry
GET    /api/deep-analysis/missions/{id}/events
GET    /api/deep-analysis/missions/{id}/costs
GET    /api/deep-analysis/missions/{id}/files
POST   /api/deep-analysis/missions/{id}/exports
...
GET    /api/knowledge/graph?spaceId={space_id}
...
아래 Phase는 선형 일정이 아니라 구현 의존성과 운영 성숙도를 나타냅니다. 2026-07-20 기준 Phase 0~2의 제품 수직 흐름은 source와 회귀 test에 구현되어 있고, Phase 3은 항목별로 구현과 Target이 섞여 있으며, Phase 4는 `deep-analysis` builtin Workspace Frontend PoC만 구현된 상태입니다.

### Phase 0. 최소 수직 흐름 — Implemented
...
### Phase 1. 핵심 채팅과 Run — Implemented
...
### Phase 2. Plan, Project와 Artifact — Implemented
...
### Phase 3. 전문 업무와 확장 — Partial

1. [Partial] PostgreSQL 호환·migration test는 구현, 실제 다중 사용자 운영 전 cutover 검증은 Target
2. [Implemented] DOCX·XLSX·PPTX·PDF·HTML 생성 및 형식별 검증. 단 외부 renderer가 없는 환경의 test skip은 실제 render 통과로 간주하지 않음
3. [Implemented] 대화 기반 실행 가능 Skill WorkingDraft, 명시 저장형 immutable version과 Skill 중심 Marketplace
4. [Target] Skill Merge·Diff·Change Request·Publish·Rollback과 위험 기반 검토
5. [Implemented] user·Project·Organization Skill Folder tree와 Skill·Folder 이동
6. [Partial] 승인된 MCP definition·Secret binding은 구현, 일반 로컬 문서 RAG 전환은 Target
7. [Partial] Extension 기본 승인·상태 정책은 구현, 조직 단위 review 확장은 Target
8. [Target] Plugin authoring·sandbox는 Skill·MCP 운영 후 별도 검증
...
1. [Target] persistent·user-managed Execution Environment
2. [Target] Local Workspace Bridge
3. [Target] Batch Fan-out
4. [Implemented] `deep-analysis` builtin Workspace Frontend PoC, 전용 Backend domain module, Node별 채팅 세션과 수동 Node·Edge Workflow
5. [Target] declarative UI와 MCP Apps sandbox
6. [Target] Computer Use
7. [Target] Redis·Object Storage·Kubernetes 기반 multi-node scale-out
8. [Target] 사용량이 입증된 Skill package Blob/Tree CAS, GC와 자동 Skill Eval
