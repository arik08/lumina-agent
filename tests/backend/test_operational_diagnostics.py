...
def test_installer_does_not_install_codex_harness_dependency() -> None:
    repository_root = Path(__file__).resolve().parents[2]
    ...
    assert "openai_codex" not in adapter
    assert "InstallCodex" not in installer
    assert "SkipCodex" not in installer
    assert '--extra", "codex' not in installer


def test_server_imports_codex_provider_without_external_codex_package() -> None:
    from lumina.main import create_app
    from lumina.providers.codex import CodexResponsesAdapter, codex_oauth_available

    assert callable(create_app)
    assert callable(codex_oauth_available)
    assert CodexResponsesAdapter.provider_id == "codex"
