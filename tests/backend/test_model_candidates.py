...
def test_model_candidates_allow_multiple_and_empty_selection(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(settings)) as client:
        ...
        selected = client.patch(
            ...
            headers=headers,
            json={
                "modelCandidates": {"codex": ["gpt-5.6-terra", "gpt-5.6-sol"]},
                "expectedRevision": cleared.json()["revision"],
            },
        )
        assert selected.status_code == 200, selected.text
        assert selected.json()["modelCandidates"] == {"codex": ["gpt-5.6-terra", "gpt-5.6-sol"]}

        restored = client.get(
            "/api/settings/current", params={"project_id": project_id}
        )
        assert restored.status_code == 200
        assert restored.json()["modelCandidates"] == {"codex": ["gpt-5.6-terra", "gpt-5.6-sol"]}
