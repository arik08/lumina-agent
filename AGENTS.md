...
- 요청받은 작업을 완료하고 필요한 검증까지 끝내면, 사용자가 별도로 요청하지 않아도 `git add .`로 변경사항을 stage하고 `git commit -m "수정사항: <핵심 변경 요약>"` 형식의 로컬 커밋을 생성합니다.
- 작업이 길어질 때는 독립적으로 검증 가능한 수정 단위가 끝날 때마다 같은 방식으로 중간 커밋을 남깁니다.
- `git add .` 전에 `git status`로 포함 대상을 확인하고 비밀값, 인증 정보, runtime data와 생성 캐시는 stage하지 않도록 먼저 ignore 또는 제외합니다.
- 원격 저장소 push는 사용자가 명시적으로 요청한 경우에만 수행합니다.
