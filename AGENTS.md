...
- 현재 요청과 기존 승인으로 범위가 정해진 작업은 조사, 구현, 복구와 검증까지 계속합니다. 가역적인 구현 선택은 기존 계약과 관례로 결정하고, 확인 가능한 정보는 직접 조사합니다.
- 충돌을 알리는 것 자체가 승인 대기를 뜻하지는 않습니다. 요청과 문서 우선순위로 기대 동작이 결정되면 승인된 범위에서 수정·검증을 계속합니다. 제품 계약, 요청 범위 또는 기존 승인 경계를 바꾸는 선택만 질문하며, 답변에 의존하지 않는 작업은 계속합니다.
- 승인은 대상·범위·버전에 연결합니다. 같은 조건에 대해 이미 받은 명시적 승인은 반복 요청하지 않습니다. 승인 대상이 달라지면 다시 확인하며, 침묵이나 시간 경과는 승인으로 간주하지 않습니다. 명시적 승인 요건과 사용자 환경 보호 규칙은 유지합니다.
...
- `primary-navigation`의 목록형 최상위 기능은 `sidebar-scroll`을 공통 생성·목록 탐색 영역으로 사용합니다. 프로젝트 선택 영역은 기능 전환과 관계없이 유지하며, 하나의 프로젝트는 채팅·심층분석 등 서로 다른 유형의 항목을 함께 포함할 수 있습니다.
- `sidebar-scroll`의 생성 action과 목록은 현재 선택한 최상위 기능에 맞게 전환합니다. 예를 들어 에이전트에서는 `새 채팅`과 채팅 목록을, 심층분석에서는 `새 분석`과 Mission 목록을 표시하며 서로 다른 유형의 항목을 한 목록에 섞지 않습니다.
- 목록 항목의 leading icon은 말풍선 같은 공통 아이콘으로 고정하지 않고 해당 기능의 식별 아이콘을 사용합니다. 프로젝트를 변경해도 현재 최상위 기능을 유지한 채 새 프로젝트 범위의 목록을 표시하며, 같은 목록을 본문 pane에 중복 배치하지 않습니다.
- 이름·라벨·제목은 가능한 한 보이는 자리에서 직접 편집하고 저장·취소·오류도 같은 위치에 표시합니다.
- 삭제는 popup이나 modal 대신 같은 버튼의 인라인 2단계 확인을 사용합니다. 대상이나 화면이 바뀌면 확인 상태를 해제합니다. 이는 제품 UI의 interaction 계약이며, 개발 작업마다 사용자에게 두 번 승인받으라는 의미가 아닙니다.
...
- CodeGraph MCP와 CLI를 모두 사용할 수 없거나 갱신이 실패하면 원인에 맞는 제한된 복구를 시도한 뒤 `rg`, source·호출부 조사와 관련 test로 영향 분석을 계속합니다. 같은 오류를 변화 없이 반복하지 않으며 Graph 미사용 사실과 남은 분석 한계를 보고합니다. 별도 승인이 필요한 설치나 사용자 환경 변경은 자동 수행하지 않습니다.
