...
- `primary-navigation`의 최상위 기능 중 목록을 제공하는 기능은 `sidebar-scroll`을 공통 목록 탐색 영역으로 사용합니다. `sidebar-scroll`을 에이전트 전용 영역으로 취급하거나 다른 기능에서 비워 두지 않고, 현재 선택한 최상위 기능에 맞는 목록을 표시합니다.
